...
function pedagogicCoverage() {
  const workspaces = readWorkspaces(); const children = (workspaces.workspaces || []).find(item => item.id === 'children'); const library = (children?.activities || []).filter(item => item?.title && item.title !== 'Activitate nouă'); const themed = readBeckyThemedActivities().filter(item => item.status === 'active');
  ...
  return { library: build(library, 'library'), themed: build(themed, 'themed'), themed_activities: themed, library_activities: library.map(item => ({ id: item.id, title: item.title, category: item.category || 'Gândește', age_categories: pedagogicActivityAges(item), participant_categories: pedagogicActivityParticipants(item), implementation: item.difficulty || '' })) };
}
