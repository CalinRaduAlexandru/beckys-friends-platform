...
const CRM_CHILD_COLUMNS = 'id,first_name,age,interests,continuity,game_recommendations,created_at,updated_at';
...
function normalizeCrmChildInput(input) {
  ...
  const game_recommendations = String(input?.game_recommendations || '').trim();
  if (!id || !first_name || !Number.isInteger(age) || age < 0 || age > 18 || interests.length > 1000 || continuity.length > 1000 || game_recommendations.length > 5000) throw Object.assign(new Error('CRM child invalid'), { status: 400 });
  return { id, first_name, age, interests, continuity, game_recommendations };
}
