...
export const PROPOSAL_STATUSES = ['pending', 'approved', 'ignored', 'failed'];
...
export function resolveAiProposal(raw, context) {
  ...
  const source_excerpt = clean(raw?.source_excerpt);
  ...
  const payload = {}; const field_provenance = {}; let target_entity_type = null; let target_entity_id = null; let target_candidates = [];
  if (destination === 'activity_observation') {
    target_entity_type = 'activity'; const matches = exactMatches(targetName, context.activities || [], 'title'); target_candidates = matches.map(item => ({ id: item.id, label: item.title })); if (matches.length === 1) target_entity_id = matches[0].id;
    ...
    const explicitAges = Array.isArray(suggested.age_categories) ? suggested.age_categories.filter(value => ACTIVITY_AGES.includes(value)) : [];
    ...
    payload.age_categories = explicitAges.length ? explicitAges : derivedAges; field_provenance.age_categories = provenance(explicitAges.length ? sourceFor(stated, 'age_categories', 'note') : derivedAges.length ? 'system' : 'missing', explicitAges.length ? '' : derivedAges.length ? 'Derivat din vârstele copiilor identificați' : 'Selectează vârsta');
    payload.participants = ACTIVITY_PARTICIPANTS.includes(suggested.participants) ? suggested.participants : participantBucket(suggested.participant_count); field_provenance.participants = provenance(payload.participants ? sourceFor(stated, 'participants', 'note') : 'missing');
  ...
  return { destination, operation, source_excerpt, target_entity_type, target_entity_id, target_candidates, payload, field_provenance };
}
