...
export const MEMORY_EPISTEMIC_TYPES = ['observed', 'direct_quote', 'activity_evidence', 'action_intent'];
...
export const isSpeculativeMemorySignal = value => /(?:consider(?:atie|a)? ca|cred ca|probabil|ar putea|ar aprecia|ar trebui|mi se pare|presupun|posibil ca|s[- ]ar putea|ne[- ]ar|ar fi(?: bine)?)/i.test(normal(value));
// A factual signal is not automatically a task. Only explicit intent, an open
// action, or a concrete follow-up is eligible to be offered as a task.
export const isTaskCandidateMemorySignal = value => /(?:intentia|vreau sa|vrem sa|trebuie sa|ne propunem sa|de documentat|de verificat|de pregatit|de creat|de construit|de rezolvat|de testat|sa documentam|sa verificam|sa pregatim|sa cream|sa construim|sa testam|data viitoare|urmatorul pas|ramane de|follow[- ]?up)/i.test(normal(value));
...
export function resolveMemorySignal(raw, context) {
  ...
  if(!normalized_observation || isSpeculativeMemorySignal(exact_source_excerpt) || isSpeculativeMemorySignal(normalized_observation)) return null;
...
// The note version is deliberately ignored here: editing a note must not turn
// the same extracted fragment into a second memory signal.
export function memorySignalContentMatches(left, right) {
  return String(left?.source_note_id || '') === String(right?.source_note_id || '')
    && normal(left?.exact_source_excerpt) === normal(right?.exact_source_excerpt);
}
...
export function selectAttentionCandidates(rawCandidates, { signals = [], noteDate = '' } = {}) {
  ...
  for(const raw of Array.isArray(rawCandidates)?rawCandidates:[]) {
    ...
    const explicitAction = /(?:intentia|vreau sa|vrem sa|trebuie sa|ne propunem sa|de documentat|de verificat|de pregatit|de creat|de construit|de rezolvat|de testat|sa documentam|sa verificam|sa pregatim|sa cream|sa construim|sa testam|data viitoare|urmatorul pas|ramane de|plan clar|imi lipsesc)/i.test(normal(`${title} ${summary} ${suggested_next_step}`));
    const historicalPattern = evidence.length>=2 && dates.size>=2 && hasHistorical;
    if(!title||!summary||!why_it_matters||!suggested_next_step||(!historicalPattern&&!explicitAction)||relevance_score<(explicitAction?60:45)||confidence<(explicitAction?.6:.55)||(merelyRepeatsEvidence&&!explicitAction))continue;
