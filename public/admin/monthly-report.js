(function () {
  ...
  const repertoireCoverage = count => count === 0 ? ["Gol", "empty"] : count === 1 ? ["Fragil", "fragile"] : count <= 3 ? ["În construcție", "building"] : ["Bine acoperit", "covered"];
  function repertoireCell(items, age, stage, activityCount, previews) {
    if (stage === "activities") return `<div class="experience-repertoire-cell experience-repertoire-activities"><strong>${activityCount} ${activityCount === 1 ? "activitate" : "activități"}</strong><small>din Biblioteca Copii</small><a href="/admin/biblioteca-activitati-copii">Deschide Biblioteca →</a>${previews.length ? `<span>${previews.map(item => esc(item.title)).join(" · ")}</span>` : ""}</div>`;
    const relevant = items.filter(item => item.age_groups?.includes(age) && item.stage === stage); const [coverage, tone] = repertoireCoverage(relevant.length);
    return `<div class="experience-repertoire-cell coverage-${tone}" data-repertoire-cell data-age="${age}" data-stage="${stage}"><header><strong>${relevant.length} ${relevant.length === 1 ? "idee" : "idei"}</strong><small>${coverage}</small></header><div class="experience-repertoire-chips">${relevant.slice(0, 3).map(item => `<button type="button" data-repertoire-edit="${esc(item.id)}">${esc(item.title)}</button>`).join("")}</div><footer><button type="button" data-repertoire-view data-age="${age}" data-stage="${stage}">Vezi toate</button><button type="button" data-repertoire-add data-age="${age}" data-stage="${stage}">＋</button></footer></div>`;
  }
  function experienceRepertoireMarkup() { return `<section class="experience-repertoire" id="experience-repertoire"><header class="experience-repertoire-head"><div><small>EXPERIENȚA COPILULUI</small><h3>Repertoriul experienței copilului</h3><p>Opțiuni reutilizabile pentru fiecare vârstă, nu un protocol rigid.</p></div><button type="button" data-repertoire-add>＋ Adaugă idee</button></header><div class="experience-repertoire-matrix" data-repertoire-matrix><p class="monthly-report-empty">Se încarcă repertoriul…</p></div><div class="experience-repertoire-modal" data-repertoire-modal hidden></div></section>`; }
  async function loadExperienceRepertoire(app) {
    if (!document.querySelector('link[data-experience-repertoire-css]')) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = '/admin/experience-repertoire.css?v=20260820-1'; link.dataset.experienceRepertoireCss = 'true'; document.head.appendChild(link); }
