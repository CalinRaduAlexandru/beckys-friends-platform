(function () {
  ...
  function pedagogicMatrix(source, data, domain) {
    ...
    const title = source === "library" ? "Biblioteca Becky" : "Activități tematice Becky";
    const note = source === "library" ? "Activități din Biblioteca Becky, cu validare separată pentru fiecare combinație vârstă × participanți." : "Activități create pentru facilitatori, cu validare separată pentru fiecare combinație.";
  ...
  async function loadPedagogicCoverage(app) {
    if (!document.querySelector('link[data-experience-repertoire-css]')) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = '/admin/experience-repertoire.css?v=20260820-4'; link.dataset.experienceRepertoireCss = 'true'; document.head.appendChild(link); }
    ...
    const openLibraryValidation = (candidates, age, participant) => { modal.hidden = false; modal.innerHTML = `<div class="experience-repertoire-dialog"><button type="button" data-themed-close>×</button><small>VALIDARE BIBLIOTECA COPII</small><h4>${age} · ${participant}</h4><p>Alege statusul canonic pentru fiecare activitate din această celulă.</p><form>${candidates.map(item => { const current = (item.validations || []).find(v => v.age_category === age && v.participant_category === participant)?.validation_status || 'idea'; return `<label class="library-validation-row"><strong>${esc(item.title)}</strong><select data-library-validation="${esc(item.id)}"><option value="idea" ${current === 'idea' ? 'selected' : ''}>💡 Idee</option><option value="validated" ${current === 'validated' ? 'selected' : ''}>✓ Validată</option></select></label>`; }).join('')}<div class="experience-repertoire-form-actions"><button type="button" data-themed-close>Anulează</button><button class="primary" type="submit">Salvează statusurile</button></div></form></div>`; modal.querySelectorAll('[data-themed-close]').forEach(button => button.onclick = () => { modal.hidden = true; }); modal.querySelector('form').onsubmit = async event => { event.preventDefault(); for (const select of modal.querySelectorAll('[data-library-validation]')) await api('/api/admin/children-activity-validations', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ activity_id:select.dataset.libraryValidation, age_category:age, participant_category:participant, validation_status:select.value }) }); modal.hidden = true; const refreshed = await api('/api/admin/pedagogic-coverage'); Object.assign(data, await refreshed.json()); renderMatrices(); }; };
    const bind = () => { app.querySelectorAll('[data-themed-add]').forEach(button => button.onclick = () => openEditor()); app.querySelectorAll('[data-pedagogic-cell]').forEach(button => button.onclick = () => { const candidates = (button.dataset.source === 'themed' ? (data.themed_activities || []) : (data.library_activities || [])).filter(candidate => (candidate.age_categories || []).includes(button.dataset.age) && (candidate.participant_categories || []).includes(button.dataset.participant)); if (button.dataset.source === 'themed') { if (candidates[0]) openEditor(candidates[0]); return; } if (candidates.length) openLibraryValidation(candidates, button.dataset.age, button.dataset.participant); else window.location.href = '/admin/biblioteca-activitati-copii'; }); };
  ...
  function renderNotes(report) {
    ...
    app.innerHTML = `<section class="monthly-report-shell"><div class="monthly-report-header"><div><small class="monthly-report-eyebrow">ADMIN · RAPORT LUNAR V1</small><h1>Note zilnice</h1><p>Un jurnal comun pentru toate rolurile. Poți folosi <strong>@rol</strong> ca să marchezi contextul unei note.</p></div><a class="monthly-report-back" href="/admin?view=monthly-report">← Înapoi la roluri</a></div><div class="monthly-report-notes-layout"><form class="monthly-report-editor monthly-report-note-form"><label>Data<input id="daily-note-date" type="date" value="${today()}"></label><label>Notă zilnică<textarea id="daily-note-text" placeholder="Ex. @design-pedagogic Am testat..."></textarea></label><div id="daily-note-analysis-state"></div><div class="monthly-report-actions"><span id="daily-note-message"></span><button type="button" id="daily-note-analyze">✨ Analizează nota</button><button class="primary" type="submit">Salvează nota</button></div></form><section class="monthly-report-editor"><div class="monthly-report-editor-head"><h2>Jurnal</h2><a class="monthly-report-tab-link" href="/admin?view=becky-inbox">Deschide Becky Inbox →</a></div><div class="monthly-report-notes-list">${
      Object.entries(report.notes || {})
        ...
        .map(
          ([date, text]) =>
            `<article tabindex="0" data-note-date="${esc(date)}"><time>${esc(date)}</time><p>${esc(text)}</p><button type="button" class="monthly-note-story" data-note-story="${esc(date)}">✦ Povestea de azi →</button></article>`,
        )
    ...
    app.querySelectorAll("[data-note-story]").forEach((button) => {
      button.addEventListener("click", (event) => {
        event.stopPropagation();
        location.href = `/admin?view=content&story_date=${encodeURIComponent(button.dataset.noteStory)}`;
      });
    });
    app
