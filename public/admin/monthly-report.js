(function () {
  ...
  async function loadPedagogicCoverage(app) {
    ...
    const bind = () => { app.querySelectorAll('[data-themed-add]').forEach(button => button.onclick = () => openEditor()); app.querySelectorAll('[data-pedagogic-cell]').forEach(button => button.onclick = async () => { const candidates = (button.dataset.source === 'themed' ? (data.themed_activities || []) : (data.library_activities || [])).filter(candidate => (candidate.age_categories || []).includes(button.dataset.age) && (candidate.participant_categories || []).includes(button.dataset.participant)); if (button.dataset.source === 'themed') { if (candidates[0]) openEditor(candidates[0]); return; } if (!candidates.length) { window.location.href = '/admin/biblioteca-activitati-copii'; return; } for (const item of candidates) { const current = (item.validations || []).find(v => v.age_category === button.dataset.age && v.participant_category === button.dataset.participant)?.validation_status || 'idea'; const next = window.prompt(`Status pentru „${item.title}” (idea / validated)`, current); if (!next || !['idea','validated'].includes(next.trim())) continue; await api('/api/admin/children-activity-validations', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ activity_id:item.id, age_category:button.dataset.age, participant_category:button.dataset.participant, validation_status:next.trim() }) }); } const refreshed = await api('/api/admin/pedagogic-coverage'); Object.assign(data, await refreshed.json()); renderMatrices(); }); };
