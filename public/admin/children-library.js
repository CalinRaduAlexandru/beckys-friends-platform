(() => {
  ...
  function playlistView() {
    ...
    return `<div class="activity-playlist-view"><button type="button" class="library-back-to-results" data-playlists-back>← Înapoi la liste</button><header><small>LISTĂ DE ACTIVITĂȚI</small><h1>${safe(list?.title || 'Listă')}</h1><p>${items.length} ${items.length === 1 ? 'activitate' : 'activități'}</p></header><div class="activity-playlist-items">${items.map((activity, index) => { const description = String(activity.subtitle || String(activity.steps || '').split(/\n+/)[0] || 'Activitate Becky').replace(/\s+/g, ' ').trim(); return `<article class="activity-playlist-item" draggable="true" data-playlist-item="${safe(activity.id)}" data-playlist-index="${index}"><span class="playlist-drag">☷</span><div><strong>${safe(activity.title)}</strong><p class="playlist-activity-description">${safe(description)}</p><small>${safe(activity.category || 'Activitate')}</small></div><button type="button" data-playlist-open="${safe(activity.id)}">Citește →</button><button type="button" data-playlist-remove="${safe(activity.id)}" aria-label="Elimină">×</button></article>`; }).join('') || '<div class="library-empty"><strong>Lista este goală</strong><p>Adaugă activități din Biblioteca Becky.</p></div>'}</div><button type="button" class="playlist-delete" data-delete-activity-playlist="${safe(list?.id || '')}">Șterge lista</button></div>`;
  }
