...
function musicGameMarkup(){const tracks=musicGameTracks();const track=tracks.find(item=>item.id===state.musicGame.trackId)||tracks[0];return `<section class="music-game-overlay ${state.musicGame.playing?'is-playing':''}" data-music-game><button type="button" class="music-game-exit" data-close-music-game>Ieși</button><button type="button" class="music-game-next" data-next-music-game>Următoarea melodie</button><div class="music-game-center"><span class="eyebrow">SCAUNELE MUZICALE</span><h1>${esc(track?.title||'Alege muzica')}</h1><div class="music-game-orbit" aria-hidden="true"><i>♪</i><i>♫</i><i>♬</i><span></span><span></span></div><button type="button" class="music-game-toggle ${state.musicGame.playing?'is-playing':''}" data-toggle-music-game><span class="music-game-toggle-icon">${state.musicGame.playing?'Ⅱ':'▶'}</span><strong>${state.musicGame.playing?'Oprește':'Pornește'}</strong><small>${state.musicGame.playing?'Muzica merge':'Apasă ca să înceapă'}</small></button></div></section>`;}
