...
function startActivityFromLauncher() { const launch=state.activityLaunch;const activity=state.activities.find(item=>item.id===launch?.activityId);if(!activity)return;state.activityLaunch=null;state.selectedActivity=activity;state.previousView='playlist';state.session={activity,steps:lines(activity.steps).length?lines(activity.steps):[activity.title],step:0};state.view='session';render();setTimer(launch.duration); }
