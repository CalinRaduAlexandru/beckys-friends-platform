...
const TIMER_VOICE_CACHE = 'becky-timer-voices-v1';
...
const state = {
  ...
  timerAnnouncedMilestones: new Set(),
  timerReturn: null,
  activityLaunch: null,
  musicGame: null,
  timerId: null,
...
function shell(content, options = {}) {
  const player = state.musicGame ? '' : nowPlaying();
  return `<main class="app-shell ${player ? 'has-player' : ''}">${content}</main>${player}${timerBanner()}${state.timerFullscreen ? timerFullscreenMarkup() : ''}${state.activityLaunch ? activityLauncherMarkup() : ''}${state.musicGame ? musicGameMarkup() : ''}${options.nav === false ? '' : bottomNav()}${state.sheet ? sheetMarkup(state.sheet) : ''}${state.challenge ? challengeMarkup() : ''}${state.colorGame ? colorGameMarkup() : ''}`;
}
...
function activityPlaylistView() { const list=state.activityPlaylists.find(item=>item.id===state.selectedPlaylistId); const items=(list?.activityIds||[]).map(id=>state.activities.find(item=>item.id===id)).filter(Boolean); return shell(`<section class="detail-sheet"><div class="detail-top"><button class="back-button" data-back type="button">←</button><span class="eyebrow">LISTĂ DE ACTIVITĂȚI</span></div><section class="detail-hero"><h1>${esc(list?.title||'Listă')}</h1><p>${items.length} activități pregătite pentru echipă.</p></section><div class="game-list activity-playlist-items">${items.map(activity=>`<div class="activity-playlist-item ${launchModeForActivity(activity.id)?'has-double-tap-action ':''}${(list.completedIds||[]).includes(activity.id)?'is-complete':''}" data-playlist-item="${esc(activity.id)}" draggable="true"><span class="swipe-remove">✓ Gata</span>${gameCard(activity)}</div>`).join('')||'<div class="empty">Lista este goală.</div>'}</div></section>`); }

const playlistLaunchModes = {'activity-scaunele-muzicale':'music','activity-sacul-cu-bile-colorate':'timer'};
function launchModeForActivity(id){return playlistLaunchModes[id] || null;}
function musicGameTracks(){const preferred=['waka','apt','shakira','chopa','chop'];const matches=preferred.map(word=>state.musicTracks.find(track=>normalize(`${track.title} ${track.artist}`).includes(word))).filter(Boolean);return [...new Map([...matches,...state.musicTracks].map(track=>[track.id,track])).values()].slice(0,3);}
function activityLauncherMarkup(){const activity=state.activities.find(item=>item.id===state.activityLaunch?.activityId);if(!activity)return '';const mode=state.activityLaunch.mode;if(mode==='music'){const tracks=musicGameTracks();return `<section class="activity-launch-overlay" data-activity-launch-overlay><div class="activity-launch-card"><button type="button" class="challenge-close" data-close-activity-launch>×</button><span class="eyebrow">SCAUNELE MUZICALE</span><h1>Alege muzica</h1><p>Muzica de fundal se oprește când alegi o melodie pentru joc.</p><div class="activity-launch-tracks">${tracks.map((track,index)=>`<button type="button" data-launch-track="${esc(track.id)}"><span>${index+1}</span><strong>${esc(track.title)}</strong><small>${esc(track.artist||'Melodie Becky')}</small><i>▶</i></button>`).join('')||'<div class="empty">Nu sunt încă melodii disponibile.</div>'}</div></div></section>`;}const durations=[60,120,180];return `<section class="activity-launch-overlay" data-activity-launch-overlay><div class="activity-launch-card"><button type="button" class="challenge-close" data-close-activity-launch>×</button><span class="eyebrow">SACUL CU BILE COLORATE</span><h1>Alege durata jocului</h1><p>Muzica rămâne să cânte în fundal.</p><div class="activity-launch-options">${durations.map(value=>`<button type="button" class="${state.activityLaunch.duration===value?'is-selected':''}" data-launch-duration="${value}">${value/60===1?'1 minut':`${value/60} minute`}</button>`).join('')}</div><button type="button" class="activity-launch-start" data-launch-start>Pornește jocul</button></div></section>`;}
function musicGameMarkup(){const tracks=musicGameTracks();const track=tracks.find(item=>item.id===state.musicGame.trackId)||tracks[0];return `<section class="music-game-overlay ${state.musicGame.playing?'is-playing':''}" data-music-game><button type="button" class="music-game-exit" data-close-music-game>Ieși</button><button type="button" class="music-game-next" data-next-music-game>Următoarea melodie</button><div class="music-game-center"><span class="eyebrow">SCAUNELE MUZICALE</span><h1>${esc(track?.title||'Alege muzica')}</h1><div class="music-game-orbit" aria-hidden="true"><div class="music-game-orbit-spin"><i>♪</i><i>♫</i><i>♬</i><span></span><span></span></div></div><button type="button" class="music-game-toggle ${state.musicGame.playing?'is-playing':''}" data-toggle-music-game><span class="music-game-toggle-icon">${state.musicGame.playing?'Ⅱ':'▶'}</span><strong>${state.musicGame.playing?'Oprește':'Pornește'}</strong><small>${state.musicGame.playing?'Muzica merge':'Apasă ca să înceapă'}</small></button></div></section>`;}
...
function toolsView() {
  return shell(`${header('Trusă')}<section class="hero-card"><small>INSTRUMENTE DE FACILITARE</small><h1>Puține gesturi. Efect imediat.</h1><p>Sunetele, timerul și provocările sunt instrumente scurte. Jocul și copiii rămân în centru.</p></section><section class="section"><div class="section-heading"><div><span class="eyebrow">REACȚII RAPIDE</span><h2>O singură atingere</h2></div></div>${effectsGrid()}</section><section class="section"><div class="section-heading"><div><span class="eyebrow">RITM</span><h2>Timer rapid</h2></div></div><div class="timer-options">${[15,30,60,180,300].map(value => `<button data-set-timer="${value}">${value < 60 ? `${value} sec` : `${value / 60} min`}</button>`).join('')}</div></section>${timerVoicePreviewMarkup()}<section class="section"><button class="start-game" data-open-color-game type="button">Alege o culoare prin shake</button></section><section class="section"><button class="start-game" data-open-challenge type="button">Deschide provocările</button></section>`);
}
...
function sheetMarkup(kind) {
  ...
  if (kind === 'timer') body = `<div class="timer-options">${[15,30,60,180,300].map(value => `<button data-set-timer="${value}">${value < 60 ? `${value} sec` : `${value / 60} min`}</button>`).join('')}</div>${state.timerInitial ? `<button class="start-game" data-toggle-timer>${state.timerRunning ? 'Pauză' : state.timerSeconds ? 'Continuă' : 'Repornește'}</button>` : ''}`;
...
function bind() {
  ...
  root.querySelectorAll('[data-close-activity-launch]').forEach(button => button.addEventListener('click', closeActivityLauncher));
  root.querySelectorAll('[data-launch-duration]').forEach(button => button.addEventListener('click', () => { if (state.activityLaunch) { state.activityLaunch.duration=Number(button.dataset.launchDuration); render(); } }));
  root.querySelectorAll('[data-launch-track]').forEach(button => button.addEventListener('click', () => startMusicGame(button.dataset.launchTrack)));
  root.querySelector('[data-launch-start]')?.addEventListener('click', startActivityFromLauncher);
  root.querySelector('[data-toggle-music-game]')?.addEventListener('click', toggleMusicGame);
  root.querySelector('[data-next-music-game]')?.addEventListener('click', nextMusicGameTrack);
  root.querySelector('[data-close-music-game]')?.addEventListener('click', closeMusicGame);
...
function bindPlaylistItem(item) { let startX=0; let startY=0; let moved=false; let lastTap=0; let tapTimer=null; let suppressClickUntil=0; const activate=()=>{const game=item.querySelector('[data-game]');if(!game)return;const now=Date.now();if(now-lastTap<340){clearTimeout(tapTimer);lastTap=0;const mode=launchModeForActivity(game.dataset.game);if(mode)openActivityLauncher(game.dataset.game,mode);}else{lastTap=now;tapTimer=setTimeout(()=>{lastTap=0;openActivity(game.dataset.game,true);},340);}}; item.addEventListener('click',event=>{if(Date.now()<suppressClickUntil)return;if(event.target.closest('button'))activate();}); item.addEventListener('touchstart',event=>{startX=event.changedTouches[0].screenX;startY=event.changedTouches[0].screenY;moved=false;item.classList.add('is-pressing');},{passive:true}); item.addEventListener('touchmove',event=>{const touch=event.changedTouches[0];const dx=touch.screenX-startX;const dy=touch.screenY-startY;if(Math.abs(dx)>12||Math.abs(dy)>12)moved=true;if(Math.abs(dx)>8&&Math.abs(dx)>Math.abs(dy)){event.preventDefault();item.style.transform=`translateX(${Math.max(-100,Math.min(100,dx))}px)`;item.classList.toggle('is-swipe-ready',dx< -70);}}, {passive:false}); item.addEventListener('touchend',event=>{const dx=event.changedTouches[0].screenX-startX;item.classList.remove('is-pressing');item.style.transform='';suppressClickUntil=Date.now()+500;if(dx< -70){toggleActivityDone(item.dataset.playlistItem);return;}if(!moved)activate();}); item.addEventListener('dragstart',event=>{event.dataTransfer.setData('text/plain',item.dataset.playlistItem);item.classList.add('is-dragging');});item.addEventListener('dragend',()=>item.classList.remove('is-dragging'));item.addEventListener('dragover',event=>event.preventDefault());item.addEventListener('drop',event=>{event.preventDefault();reorderPlaylistItem(event.dataTransfer.getData('text/plain'),item.dataset.playlistItem);}); }
function toggleActivityDone(activityId){const list=state.activityPlaylists.find(item=>item.id===state.selectedPlaylistId);if(!list)return;list.completedIds=[...(list.completedIds||[])];const index=list.completedIds.indexOf(activityId);const completed=index<0;if(completed)list.completedIds.push(activityId);else list.completedIds.splice(index,1);render();saveActivityPlaylists(null,completed?'Activitate bifată ✓':'Activitate debifată ✓');}
function reorderPlaylistItem(fromId,toId){const list=state.activityPlaylists.find(item=>item.id===state.selectedPlaylistId);if(!list||fromId===toId)return;const ids=[...(list.activityIds||[])];const from=ids.indexOf(fromId),to=ids.indexOf(toId);if(from<0||to<0)return;ids.splice(to,0,ids.splice(from,1)[0]);list.activityIds=ids;render();saveActivityPlaylists(null,'Ordinea listei a fost actualizată ✓');}
...
function openActivity(id, fromPlaylist=false) { state.selectedActivity = state.activities.find(item => item.id === id); state.previousView = fromPlaylist ? 'playlist' : 'games'; state.view = 'detail'; state.recentIds = [id, ...state.recentIds.filter(value => value !== id)].slice(0,8); saveJson(RECENT_KEY,state.recentIds); render(); window.scrollTo(0,0); }
function openActivityLauncher(id,mode=launchModeForActivity(id)) { if(!mode)return; state.activityLaunch={activityId:id,mode,duration:mode==='timer'?120:60}; render(); }
function closeActivityLauncher() { state.activityLaunch=null; render(); }
function startActivityFromLauncher() { const launch=state.activityLaunch;const activity=state.activities.find(item=>item.id===launch?.activityId);if(!activity)return;state.timerReturn={view:'playlist',playlistId:state.selectedPlaylistId,activityId:activity.id};state.activityLaunch=null;state.selectedActivity=activity;state.previousView='playlist';state.session={activity,steps:lines(activity.steps).length?lines(activity.steps):[activity.title],step:0};state.view='session';render();setTimer(launch.duration); }
function startMusicGame(trackId) { const activity=state.activities.find(item=>item.id==='activity-scaunele-muzicale');state.activityLaunch=null;state.currentTrack=null;state.currentPlaylist=null;audio.pause();state.musicGame={activityId:activity?.id,trackId,playing:false};render(); }
function playMusicGameTrack(track){if(!track)return;state.currentTrack=track;state.currentPlaylist=null;audio.pause();audio.src=track.src;audio.volume=Number(localStorage.getItem(VOLUME_KEY)||.35);audio.load();audio.play().catch(()=>{});}
function updateMusicGameDOM(){const overlay=root.querySelector('[data-music-game]');if(!overlay)return;const track=selectedTrack(state.musicGame.trackId);overlay.classList.toggle('is-playing',state.musicGame.playing);const button=overlay.querySelector('[data-toggle-music-game]');button?.classList.toggle('is-playing',state.musicGame.playing);if(button){button.querySelector('.music-game-toggle-icon').textContent=state.musicGame.playing?'Ⅱ':'▶';button.querySelector('strong').textContent=state.musicGame.playing?'Oprește':'Pornește';button.querySelector('small').textContent=state.musicGame.playing?'Muzica merge':'Apasă ca să înceapă';button.classList.remove('is-changing');void button.offsetWidth;button.classList.add('is-changing');}if(track)overlay.querySelector('.music-game-center h1').textContent=track.title;}
function toggleMusicGame() { if(!state.musicGame)return;const track=selectedTrack(state.musicGame.trackId);if(!track)return;if(state.musicGame.playing){audio.pause();state.musicGame.playing=false;}else{state.musicGame.playing=true;if(state.currentTrack?.id===track.id&&audio.src){audio.play().catch(()=>{});}else playMusicGameTrack(track);}updateMusicGameDOM(); }
function nextMusicGameTrack() { if(!state.musicGame)return;const tracks=musicGameTracks();const index=Math.max(0,tracks.findIndex(track=>track.id===state.musicGame.trackId));const next=tracks[(index+1)%tracks.length];if(next){state.musicGame.trackId=next.id;state.musicGame.playing=true;playMusicGameTrack(next);updateMusicGameDOM();}}
function closeMusicGame() { audio.pause();state.currentTrack=null;state.currentPlaylist=null;state.musicGame=null;render(); }
async function saveActivityPlaylists(afterSave, message='Lista a fost actualizată ✓') { if(!facilitatorSession?.access_code)return; const response=await fetch('/api/facilitator/playlists',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:facilitatorSession.access_code,playlists:state.activityPlaylists})}); if(response.ok){const payload=await response.json();state.activityPlaylists=payload.activityPlaylists||state.activityPlaylists;saveJson(CACHE_KEY,{...readJson(CACHE_KEY,{}),workspaces:readJson(CACHE_KEY,{}).workspaces});toast(message,afterSave);}else toast('Lista nu a putut fi salvată.'); }
function completeTimerActivity(){const target=state.timerReturn;if(!target?.playlistId||!target.activityId)return;const list=state.activityPlaylists.find(item=>item.id===target.playlistId);if(!list)return;list.completedIds=[...(list.completedIds||[])];if(list.completedIds.includes(target.activityId))return;list.completedIds.push(target.activityId);saveActivityPlaylists(null,'Activitate completată ✓');}
...
function startTimerInterval(){timerCountdownStarted=false;clearInterval(state.timerId);state.timerId=setInterval(()=>{state.timerSeconds-=1;updateTimerDOM();if(state.timerSeconds===120&&state.timerInitial>=180&&!state.timerAnnouncedMilestones.has(120)){state.timerAnnouncedMilestones.add(120);playTimerVoiceText('Mai sunt două minute!');}if(state.timerSeconds===60&&state.timerInitial>=120&&!state.timerAnnouncedMilestones.has(60)){state.timerAnnouncedMilestones.add(60);playTimerVoiceText('Mai este un minut!');}if(state.timerSeconds===10&&!timerCountdownStarted)playTimerCountdownAudio();if(state.timerSeconds<=0){completeTimerActivity();clearTimer(false);updateTimerFullscreenControls();playTimerFinish();toast('Timpul s-a terminat.');updateTimerDOM();}},1000);}
...
function setTimer(seconds){clearInterval(state.timerId);clearTimerVoiceClips();state.timerAnnouncedMilestones.clear();state.timerInitial=seconds;state.timerSeconds=seconds;state.timerRunning=false;state.timerAnnouncing=true;state.timerVoiceError=false;state.timerAnnouncementText=timerAnnouncement(seconds);state.sheet='';state.timerFullscreen=true;render();requestTimerFullscreen();announceAndStartTimer(seconds);}
async function announceAndStartTimer(seconds){const preparation=prepareTimerVoiceClips();const voiceFinished=await playTimerVoiceText(state.timerAnnouncementText);await preparation;if(!state.timerFullscreen||state.timerInitial!==seconds||state.timerSeconds!==seconds)return;if(!voiceFinished){state.timerVoiceError=true;setTimerVoiceErrorUI();return;}activateTimerAfterAnnouncement();}
async function retryTimerVoice(){if(!state.timerAnnouncementText)return;state.timerVoiceError=false;setTimerAnnouncingUI();const voiceFinished=await playTimerVoiceText(state.timerAnnouncementText);if(!state.timerFullscreen||!state.timerAnnouncing)return;if(!voiceFinished){state.timerVoiceError=true;setTimerVoiceErrorUI();return;}activateTimerAfterAnnouncement();}
function activateTimerAfterAnnouncement(){state.timerAnnouncing=false;state.timerRunning=true;startTimerInterval();const overlay=root.querySelector('[data-timer-fullscreen]');if(!overlay)return;overlay.querySelector('.timer-fullscreen-body>small').textContent='ÎN DESFĂȘURARE';const controls=overlay.querySelector('.timer-fullscreen-controls');controls.innerHTML='<button type="button" data-toggle-timer>Ⅱ  Pauză</button><button type="button" class="timer-secondary" data-reset-timer>↻  Repornește</button><button type="button" class="timer-secondary" data-close-timer>Închide</button>';controls.querySelector('[data-toggle-timer]').addEventListener('click',toggleTimer);controls.querySelector('[data-reset-timer]').addEventListener('click',resetTimer);controls.querySelector('[data-close-timer]').addEventListener('click',closeTimerFullscreen);}
function updateTimerFullscreenControls(){const overlay=root.querySelector('[data-timer-fullscreen]');if(!overlay)return;const status=overlay.querySelector('.timer-fullscreen-body>small');if(status)status.textContent=state.timerRunning?'ÎN DESFĂȘURARE':state.timerSeconds?'PUS PE PAUZĂ':'TIMPUL S-A TERMINAT';const button=overlay.querySelector('[data-toggle-timer]');if(button)button.textContent=state.timerRunning?'Ⅱ  Pauză':state.timerSeconds?'▶  Reia':'↻  Repornește';}
function setTimerAnnouncingUI(){const overlay=root.querySelector('[data-timer-fullscreen]');if(!overlay)return;overlay.querySelector('.timer-fullscreen-body>small').textContent='ASCULTĂ INSTRUCȚIUNILE';const button=overlay.querySelector('.timer-fullscreen-controls button');if(button){button.disabled=true;button.textContent='Se pregătește START…';}}
function setTimerVoiceErrorUI(){const overlay=root.querySelector('[data-timer-fullscreen]');if(!overlay)return;overlay.querySelector('.timer-fullscreen-body>small').textContent='VOCEA NU A PORNIT';const controls=overlay.querySelector('.timer-fullscreen-controls');controls.innerHTML='<button type="button" data-retry-timer-voice>▶  Reia anunțul</button><button type="button" class="timer-secondary" data-close-timer>Închide</button>';controls.querySelector('[data-retry-timer-voice]').addEventListener('click',retryTimerVoice);controls.querySelector('[data-close-timer]').addEventListener('click',closeTimerFullscreen);}
function toggleTimer(){if(state.timerRunning){clearInterval(state.timerId);state.timerId=null;state.timerRunning=false;}else{if(!state.timerSeconds)state.timerSeconds=state.timerInitial;state.timerRunning=true;startTimerInterval();}if(state.timerFullscreen){updateTimerFullscreenControls();updateTimerDOM();}else render();}
function resetTimer(){if(!state.timerInitial)return;clearInterval(state.timerId);state.timerAnnouncedMilestones.clear();state.timerSeconds=state.timerInitial;state.timerRunning=true;startTimerInterval();if(state.timerFullscreen){updateTimerFullscreenControls();updateTimerDOM();}else render();}
function clearTimer(reset=true){clearInterval(state.timerId);state.timerId=null;state.timerRunning=false;state.timerAnnouncing=false;state.timerVoiceError=false;timerCountdownStarted=false;if(reset){timerCountdownAudio?.pause();timerCountdownAudio=null;state.timerSeconds=0;state.timerInitial=0;state.timerAnnouncementText='';state.timerAnnouncedMilestones.clear();}}
...
function closeTimerFullscreen(){const returnTarget=state.timerReturn;state.timerReturn=null;state.timerFullscreen=false;clearTimer(true);const exit=document.fullscreenElement?document.exitFullscreen?.():null;const finish=()=>{if(returnTarget?.view==='playlist'){state.selectedPlaylistId=returnTarget.playlistId;state.selectedActivity=null;state.session=null;state.view='playlist';}render();if(returnTarget?.view==='playlist'){requestAnimationFrame(()=>{const item=[...root.querySelectorAll('[data-playlist-item]')].find(element=>element.dataset.playlistItem===returnTarget.activityId);item?.scrollIntoView({behavior:'smooth',block:'center'});});}};if(exit?.then)exit.then(finish).catch(finish);else finish();}
...
function timerCountdownText(){return 'zece... nouă... opt... șapte... șase... cinci... patru... trei... doi... unu...';}
function timerVoiceTexts(){return [timerCountdownText(),'Mai este un minut!','Mai sunt două minute!','GATAA! STOP JOC!'];}
async function loadTimerVoice(text){if(timerVoiceClips.has(text))return timerVoiceClips.get(text);const request=new Request(`/__timer-voice/${encodeURIComponent(text)}`);try{const cache=await caches.open(TIMER_VOICE_CACHE);const cached=await cache.match(request);if(cached){const url=URL.createObjectURL(await cached.blob());timerVoiceClips.set(text,url);return url;}const response=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});if(!response.ok)throw new Error('TTS');await cache.put(request,response.clone());const url=URL.createObjectURL(await response.blob());timerVoiceClips.set(text,url);return url;}catch{const response=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});if(!response.ok)throw new Error('TTS');const url=URL.createObjectURL(await response.blob());timerVoiceClips.set(text,url);return url;}}
async function prepareTimerVoiceClips(){await Promise.all(timerVoiceTexts().map(async text=>{try{await loadTimerVoice(text);}catch{}}));}
async function playTimerVoiceText(text){try{const src=await loadTimerVoice(text);voiceAudio?.pause();voiceAudio=new Audio(src);const finished=await new Promise(resolve=>{voiceAudio.addEventListener('ended',()=>resolve(true),{once:true});voiceAudio.addEventListener('error',()=>resolve(false),{once:true});voiceAudio.play().catch(()=>resolve(false));});return finished;}catch{toast('Vocea timerului nu s-a putut încărca.');return false;}}
function clearTimerVoiceClips(){timerVoiceClips.forEach(url=>URL.revokeObjectURL(url));timerVoiceClips.clear();}
async function playTimerCountdownAudio(){timerCountdownStarted=true;const src=timerVoiceClips.get(timerCountdownText());if(!src)return;timerCountdownAudio?.pause();timerCountdownAudio=new Audio(src);timerCountdownAudio.volume=1;timerCountdownAudio.preservesPitch=false;await new Promise(resolve=>{timerCountdownAudio.addEventListener('loadedmetadata',()=>{if(Number.isFinite(timerCountdownAudio.duration)&&timerCountdownAudio.duration>0)timerCountdownAudio.playbackRate=Math.max(.5,Math.min(2,timerCountdownAudio.duration/10));},{once:true});timerCountdownAudio.addEventListener('ended',resolve,{once:true});timerCountdownAudio.addEventListener('error',resolve,{once:true});timerCountdownAudio.play().catch(resolve);});}
function playTimerFinish(){const finish=()=>{playTimerBuzz();const src=timerVoiceClips.get('GATAA! STOP JOC!');if(src){setTimeout(()=>{timerCountdownAudio=new Audio(src);timerCountdownAudio.volume=1;timerCountdownAudio.play().catch(()=>{});},500);}};if(timerCountdownAudio&&!timerCountdownAudio.ended)timerCountdownAudio.addEventListener('ended',finish,{once:true});else finish();}
function playTimerBuzz(){const context=getAudioContext();const now=context.currentTime;[[260,0,.18],[340,.22,.18],[440,.44,.22],[300,.7,.62]].forEach(([frequency,delay,duration])=>{const oscillator=context.createOscillator();const gain=context.createGain();const start=now+delay;oscillator.type='square';oscillator.frequency.setValueAtTime(frequency,start);oscillator.frequency.linearRampToValueAtTime(frequency*.82,start+duration);gain.gain.setValueAtTime(.0001,start);gain.gain.exponentialRampToValueAtTime(.42,start+.018);gain.gain.setValueAtTime(.42,start+duration-.06);gain.gain.exponentialRampToValueAtTime(.0001,start+duration);oscillator.connect(gain).connect(context.destination);oscillator.start(start);oscillator.stop(start+duration+.02);});}
