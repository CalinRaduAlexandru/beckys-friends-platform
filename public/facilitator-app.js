...
function musicGameTracks(){const preferred=['waka','apt','chopa','chop'];const matches=preferred.map(word=>state.musicTracks.find(track=>normalize(`${track.title} ${track.artist}`).includes(word))).filter(Boolean);return [...new Map([...matches,...state.musicTracks].map(track=>[track.id,track])).values()].slice(0,3);}
...
function toggleMusicGame() { if(!state.musicGame)return;const track=selectedTrack(state.musicGame.trackId);if(!track)return;if(state.musicGame.playing){audio.pause();state.musicGame.playing=false;render();}else{state.musicGame.playing=true;playTrack(track,null);} }
function nextMusicGameTrack() { if(!state.musicGame)return;const tracks=musicGameTracks();const index=Math.max(0,tracks.findIndex(track=>track.id===state.musicGame.trackId));const next=tracks[(index+1)%tracks.length];if(next){state.musicGame.trackId=next.id;state.musicGame.playing=true;playTrack(next,null);} }
