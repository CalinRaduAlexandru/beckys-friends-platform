...
function playTimerCountdown(number){const index=10-Number(number);const src=timerVoiceClips.get(timerVoiceTexts()[index]);if(!src)return;timerCountdownAudio?.pause();timerCountdownAudio=new Audio(src);timerCountdownAudio.volume=1;timerCountdownAudio.playbackRate=1.35;timerCountdownAudio.preservesPitch=false;timerCountdownAudio.play().catch(()=>{});}
