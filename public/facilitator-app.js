...
function clearTimer(reset=true){clearInterval(state.timerId);state.timerId=null;state.timerRunning=false;state.timerAnnouncing=false;state.timerVoiceError=false;timerCountdownStarted=false;timerCountdownAudio?.pause();timerCountdownAudio=null;if(reset){state.timerSeconds=0;state.timerInitial=0;state.timerAnnouncementText='';}}
...
async function playTimerCountdownSequence(){timerCountdownStarted=true;for(const text of timerVoiceTexts().slice(0,10)){if(!state.timerFullscreen||state.timerSeconds<=0)return;const src=timerVoiceClips.get(text);if(!src)continue;timerCountdownAudio?.pause();timerCountdownAudio=new Audio(src);timerCountdownAudio.volume=1;timerCountdownAudio.playbackRate=1.45;timerCountdownAudio.preservesPitch=false;await new Promise(resolve=>{timerCountdownAudio.addEventListener('ended',resolve,{once:true});timerCountdownAudio.addEventListener('error',resolve,{once:true});timerCountdownAudio.play().catch(resolve);});}}
