...
function musicGameMarkup(){const tracks=musicGameTracks();const track=tracks.find(item=>item.id===state.musicGame.trackId)||tracks[0];return `<section class="music-game-overlay" data-music-game><button type="button" class="music-game-exit" data-close-music-game>Ieși</button><button type="button" class="music-game-next" data-next-music-game>Următoarea melodie</button><div class="music-game-center"><span class="eyebrow">SCAUNELE MUZICALE</span><h1>${esc(track?.title||'Alege muzica')}</h1><button type="button" class="music-game-toggle" data-toggle-music-game>${state.musicGame.playing?'Oprește':'Pornește'}</button><p>Apasă tu. Butonul este departe de margini ca să nu pornească accidental.</p></div></section>`;}
