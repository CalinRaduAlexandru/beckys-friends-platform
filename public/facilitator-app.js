...
function toggleMusicGame() { if(!state.musicGame)return;const track=selectedTrack(state.musicGame.trackId);if(!track)return;if(state.musicGame.playing){audio.pause();state.musicGame.playing=false;}else{state.musicGame.playing=true;playMusicGameTrack(track);}updateMusicGameDOM(); }
