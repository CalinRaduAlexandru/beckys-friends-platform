...
function resetTimer(){if(!state.timerInitial)return;clearInterval(state.timerId);state.timerSeconds=state.timerInitial;state.timerRunning=true;startTimerInterval();if(state.timerFullscreen){updateTimerFullscreenControls();updateTimerDOM();}else render();}
...
function timerVoiceTexts(){return [timerCountdownText(),'GATAA! STOP JOC!'];}
