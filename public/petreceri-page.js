(() => {
  ...
  const platterControls = platterCategories.map(([category, label]) => {
    ...
    return `<div class="party-food-choice"><label for="party-platter-${category}">${label}</label><select id="party-platter-${category}" data-platter-category="${category}" aria-label="Alegeți un ${label.toLowerCase()}"><option value="">Nu adăuga</option>${options}</select><input type="number" min="0" step="1" value="0" data-platter-quantity="${category}" aria-label="Număr de platouri ${label.toLowerCase()}"></div>`;
  }).join('');
  ...
  food.insertAdjacentHTML('beforeend', `<div class="party-food-quantities" data-platter-controls>${platterControls}</div><details class="party-detail party-menu-catalog"><summary>Vezi toate meniurile și componența platourilor</summary><div class="party-answer"><p class="party-menu-note">Prețurile și componența de mai jos sunt preluate din oferta de meniuri. Pentru alergii, adaptări și disponibilitatea exactă, confirmăm detaliile la rezervare.</p>${childrenCatalog}${catalogCategories}</div></details>`);
